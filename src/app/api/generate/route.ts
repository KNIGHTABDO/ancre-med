...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (isFeatureEnabled("gateSpans")) {
      ...
      const parsedPayload = enforceRequiredAbstentions(
      ...
      // Only spend an independent-verifier call on assertions that already
      // cleared the deterministic substring/entity check -- and only ever drop
      // the specific assertion it disagrees with, never the whole answer. A
      // second model being skeptical about one sentence is not evidence the
      // rest of the response is wrong.
      if (
        isFeatureEnabled("verifierFreshness") &&
        !isConversational &&
        verificationResult.verifiedAssertions.length > 0
      ) {
